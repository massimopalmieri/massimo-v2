...
const RATE_LIMIT = {
  MAX_REQUESTS: 3,
  WINDOW: 3600000,
  BLOCK_DURATION: 3600000 * 24,
}

export function checkRateLimit(ip: string): { allowed: boolean; error: string | null } {
  const now = Date.now()
...
export async function validateEmail(email: string) {
  const response = await fetch(`https://emailvalidation.abstractapi.com/v1/?api_key=${process.env.ABSTRACT_API_KEY}&email=${encodeURIComponent(email)}`)
